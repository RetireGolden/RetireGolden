...
| `planner-ui/src/socialSecurity/expectedPv.ts` | Local 50% spousal top-up model | **Educational-not-ledger (disclosed)**: file header, Benefits-only tab copy, and feature doc now state the annual simplified couple model and its limits; engine migration remains a follow-up |
| `planner-ui/src/socialSecurity/breakEven.ts` | Local COLA-from-62 accrual model | **Queued decision**: migrate into engine or carry an explicit educational-not-ledger marker |
| `planner-ui/src/socialSecurity/survivorSwitching.ts` | Local own-claim 62–70 clamps and switching model | **Queued decision**: migrate into engine or carry an explicit educational-not-ledger marker; clamps should consume engine claim-window constants |
...
| `planner-ui/src/planner/SsAnalysisPage.tsx` | Survivor-switching panel filtered former spouses | **Closed (partial predicates)**: `passesModeledOrdinaryWidowRecordGates` and `passesModeledSurvivingDivorcedRecordGates` from `maritalBenefits` apply the modeled record gates each path owns (ordinary nine-month duration vs surviving-divorced ten-year divorce duration, plus the shared historical pre-60 remarriage gate); neither tests current marital status, statutory duration/remarriage exceptions, or complete entitlement — `isWidowEligible` / `isSurvivingDivorcedEligible` own claimant age; claim timing is out of scope for that UI path |
...
| `planner-ui/src/planner/SocialSecuritySection.tsx` | Executable default marriage years per former-spouse kind (`divorced` / `surviving-divorced` → engine ten-year floor; `deceased` → one-year default above the ordinary nine-month survivor floor) | **Consume engine constants**: `DIVORCED_MIN_MARRIAGE_YEARS` and `SURVIVOR_MIN_MARRIAGE_YEARS` from `@retiregolden/engine/socialSecurity/maritalBenefits` (already exported; `ssAnalysis.ts` imports the divorced floor) |
