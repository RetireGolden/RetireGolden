...
[`FlyOverCoderKY/openrouter-pr-review-action@2eb560ca23346c4387828b4b9763698702094210`](https://github.com/FlyOverCoderKY/openrouter-pr-review-action/tree/2eb560ca23346c4387828b4b9763698702094210).
RetireGolden authorization validates decoded markers against that producer, not a vendored copy:

| Contract | Source |
|----------|--------|
| Finding decode (`id`, `sev`, `file`, `line`, `title`, `ev`, `st`, `m`) | [`loop.py` `_decode_finding`](https://github.com/FlyOverCoderKY/openrouter-pr-review-action/blob/2eb560ca23346c4387828b4b9763698702094210/src/or_pr_review/loop.py#L478-L518) |
| Safe relative paths for `file` | [`schema.py` `valid_review_path`](https://github.com/FlyOverCoderKY/openrouter-pr-review-action/blob/2eb560ca23346c4387828b4b9763698702094210/src/or_pr_review/schema.py#L280-L282) is a three-line compatibility predicate delegating to [`normalize_review_path`](https://github.com/FlyOverCoderKY/openrouter-pr-review-action/blob/2eb560ca23346c4387828b4b9763698702094210/src/or_pr_review/schema.py#L255-L277); its length limit is [`MAX_FILE = 500`](https://github.com/FlyOverCoderKY/openrouter-pr-review-action/blob/2eb560ca23346c4387828b4b9763698702094210/src/or_pr_review/schema.py#L26). |
| Round state: `fixed` removes an entry; `disputed` is carried; open counts | [`loop.py` `apply_round`](https://github.com/FlyOverCoderKY/openrouter-pr-review-action/blob/2eb560ca23346c4387828b4b9763698702094210/src/or_pr_review/loop.py#L195-L289) (including `open_issue_count`) |
| Ledger encode/decode envelope | [`loop.py` `_encode`](https://github.com/FlyOverCoderKY/openrouter-pr-review-action/blob/2eb560ca23346c4387828b4b9763698702094210/src/or_pr_review/loop.py#L376-L400) / [`_decode`](https://github.com/FlyOverCoderKY/openrouter-pr-review-action/blob/2eb560ca23346c4387828b4b9763698702094210/src/or_pr_review/loop.py#L437-L475) |
...
The current caller uses the [shared configuration from organization PR #43](https://github.com/RetireGolden/.github/blob/388872e9c44158075742f3f7795b161f88e524dd/.github/workflows/openrouter-code-review.yml)
with `review_profiles_enabled: true`, `review_policy: base`, and org workflow pin
`388872e9c44158075742f3f7795b161f88e524dd`. Root and nested `REVIEW.md` guidance comes from the
...
org reusable at `388872e9c44158075742f3f7795b161f88e524dd`. Reviews publish both the v1 ledger
...
[`scripts/profile_consumer.mjs`](https://github.com/RetireGolden/.github/blob/388872e9c44158075742f3f7795b161f88e524dd/scripts/profile_consumer.mjs)
