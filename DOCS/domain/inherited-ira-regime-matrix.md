...
| IRC §401(a)(9)(C); SECURE 2.0 §107 | Required beginning date: April 1 after attaining the applicable age — 70½ (born before 7/1/1949, a birth-*date* test), 72 (SECURE), 73 (attaining 72 after 12/31/2022), 75 (attaining 74 after 12/31/2032). The enacted §107 text places the born-1959 cohort in both the 73 and 75 buckets; Prop. Reg. §1.401(a)(9)-2 (REG-103529-23) resolves it to 73, but T.D. 10001 reserved the paragraph and Announcement 2026-7 defers the fix. | Age-73 tier from 2023. Born-1959 resolution: proposed only. | Controlling, except born-1959 → unsettled |
...
| Treas. Reg. §1.401(a)(9)-9(f)(2)(ii) | Transition reset: a fixed life expectancy set under the pre-2022 tables is redetermined once under the 2022 Single Life Table at the original lookup age, then reduced by one per elapsed year. Mechanics verified against the T.D. 9930 preamble (85 FR 72472, Nov. 12, 2020); `VERIFY:` the subparagraph number against the regulatory text before WS3 encodes it. | Same. | Controlling; until the pin is confirmed, any fixed expectancy first set before 2022 classifies `needs-review` |
...
| Treas. Reg. §1.408-8(c) | Spouse treat-as-own: a surviving spouse who is sole beneficiary with an unlimited withdrawal right may redesignate the account as their own ((c)(1)); a deemed election occurs on a missed beneficiary RMD or an owner-style contribution ((c)(2)); after election the spouse is owner for all Code purposes ((c)(3)). | Same. | Controlling from 2025 |
...
Match precedence: the X rows bind first. A fact set matching any X condition (pre-2020 death,
successor, non-individual, multiple beneficiaries without separate-account facts, missing or
contradictory facts) never reaches an R/S/K row; the R/S/K rows additionally require the
beneficiary to be the account's sole beneficiary.
...
| X5 | Any required fact missing or contradictory (e.g. claimed minor child aged 30; claimed ≤10-years-younger contradicted by birth years; owner birth-date precision insufficient to place death vs RBD; owner born 1959 with a post-RBD-sensitive classification) | `needs-review/unsupported` | — | — | Fact model §2 | Fail closed |

Completeness rule: the classifier is total over §2's fact space. Every combination matches exactly
...
- **Year-of-death RMD.** For a post-RBD death, year 0 of the schedule is the decedent's own
  unsatisfied RMD (Uniform Lifetime denominator at the owner's attained age), passed to the
  beneficiary under §1.408-8(e)(4)(i) — never the beneficiary's Single Life amount.
- **Spouse treat-as-own catch-up.** A late S2 election first distributes the §1.408-8(c)(1)(iv)
  hypothetical RMDs computed as if the spouse had remained beneficiary; the transition completes
  only after that catch-up is scheduled.
- **2022 table reset.** A fixed expectancy first set before 2022 (death in 2020, first
  ...
  lookup age, then reduced by one per elapsed year. The mechanics are verified against the T.D.
  9930 preamble; the subparagraph pin (§1.401(a)(9)-9(f)(2)(ii)) carries a `VERIFY:` flag in §1
  until WS3 confirms it against the regulatory text.
