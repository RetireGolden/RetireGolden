...
- **Owner living RMD start age** (`rmdStartAgeForBirthYear`; `irc-401-a-9-C-v-applicable-age`; `treas-reg-1-401-a-9-2-b-2-v-applicable-age-1959`): birth year ≤1950 → **72**; 1951–1959 → **73** (born-1959 remains contested between 73 and 75; the owner calculator selects 73); 1960+ → **75**. Birth-year lookup is the current forward-projection convention; it does not apply the July 1, 1949 70½ cut and does not claim to reach pre-required-beginning historical owner years before the represented horizon.
- **Inherited decedent RBD derivation** (`applicableAge.ts`; `treas-reg-1-401-a-9-2-b-2-ii-iii-applicable-age-70-half-and-72`; `irc-401-a-9-C-v-applicable-age`; `treas-reg-1-401-a-9-2-b-2-v-applicable-age-1959`): **70½** for born before July 1, 1949; **72** for born July 1, 1949–December 31, 1950; **73** for born 1951–1958; born **1959** retains candidate RBD dates April 1, 2033 (age 73) and April 1, 2035 (age 75) and refuses only when those candidates disagree on before/on-after (applicable age remains contested even when a comparison resolves; outside both candidate years resolve when the asserted fact agrees; same-year tie-break at a candidate RBD year); **75** for born 1960+. Month granularity applies for the 70½ tier and the July 1949 boundary.
...
- **Five-year / non-designated emptying.** When beneficiary classification is independently confirmed as non-designated and the owner died before the RBD, there is no annual minimum before the deadline year and the entire remaining interest is due by end of `deathYear+5` (`irc-401-a-9-B-ii-confirmed-non-designated-five-year-schedule`; leaf `fiveYearEmptyingRequirement`). The five-year leaf routes years after that deadline without a dollar requirement; the separate post-deadline leaf is the only pathway that prices the entire remaining benefit. Unknown trust/entity classification remains a typed refusal (`irc-401-a-9-B-ii-non-designated-beneficiary-five-year-rule`). Post-RBD non-designated cases are not that pathway.
- **Post-deadline remaining benefit (Treas. Reg. 54.4974-1(e)).** After the calendar year in which the entire remaining benefit was required to be distributed, each subsequent year's RMD is the **entire remaining benefit** — a distribution obligation, **not** a 100% excise rate (`treas-reg-54-4974-1-e-post-deadline-remaining-benefit`). Shortfall excise remains ordinary 25% / qualifying 10%; an emptied balance does not repeat tax on the original deadline amount.
- **Inherited Roth tax character.** Qualified status is independent of the RMD schedule. Nonqualified inherited Roth distributions consume remaining basis then put only earnings in ordinary income; the death exception removes the 10% additional tax (`treas-reg-1-408A-6-inherited-roth-nonqualified-earnings`). Unknown clock/basis fails closed.
- **Spousal owner election (Treas. Reg. 1.408-8(c)).** Production evaluation requires verified sole-beneficiary, unlimited-withdrawal, and direct-spouse facts. Observed beneficiary-RMD rows carry their legal deadline, observation cutoff, and provenance; an opening call accepts only rows whose deadline and observation are both before January 1, while current-year execution can use only dated completed acts. A recommendation, scheduled contribution, or modeled distribution never becomes observed evidence. A late affirmative election uses `determineSection402c2j4CatchUp`: from one current-year RMD reference balance it rounds each spouse-age Uniform-Lifetime hypothetical RMD, carries the positive prior hypothetical-minus-actual deficit into the next adjusted balance, and subtracts prior-year actual distributions. The final 2025 regulation supplies that recurrence; Treasury's 2024 proposed example is an illustration, not final text. An affirmative redesignation uses the direct-distribution counterfactual without a fictional actual rollover; a beneficiary-capacity destination is not applicable, and the owned-Roth positive branch is non-applicable. When applicable, the calculated amount treated as the current distribution's RMD must actually have been distributed before redesignation. Once a verified current-year event is effective, §1.408-8(c)(3) makes the election-year RMD an owner calculation: `planElectionYearOwnerRmdDraws` preserves the immutable beneficiary trigger/counterfactual, computes the owner requirement from the prior-Dec-31 (or explicit reference) balance, credits only completed provenance-backed history or the evidenced j(4) actual once, takes only the unpaid live-balance amount, and never refunds excess cash or feeds the revised requirement back into trigger eligibility. The final owner obligation (full requirement, accepted credit, actual settlement, and any unsatisfied remainder) is published separately from beneficiary evidence and enters the owner §4974/RMD gate; suppressed inherited Roth counterfactuals never consume basis. Death-year elections publish spouse-owner RMD 0 and keep the decedent residual on the inherited path. Mixed-year transaction tax chronology may remain typed `incomplete-spousal-election-mixed-year` when dated before/after ordering is unavailable (`treas-reg-1-402-c-2-j-4-surviving-spouse-catch-up-recurrence`; `treas-reg-1-408-8-c-3-spouse-treated-as-owner`; `gateBeneficiarySpousalElectionForAnnualCoordinator`; `simulate.spousalElectionYearOwnerRmd.test.ts`).
