...
| Evidence | `DOCS/operations/walkthroughs/<id>.json` | the rows with the engine's figures beside the hand values, the reading notes and the plan as built; written by `pnpm walkthroughs:export` in `packages/planner-ui` and held fresh by `walkthroughEvidence.test.ts`; format version 2 (version 1 plus null figures) |
