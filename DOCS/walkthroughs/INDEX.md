...
| Test | `packages/planner-ui/src/planner/examples/walkthroughs/<id>.test.ts` | holds the engine to every row: strings exactly, numbers to half a cent unless the figure's contract promises less (a value sized by bisection "to $0.01" states that tolerance, one-sided below the exact value where the bisection returns the lower bound; a fixed point accepted within half a cent of its residual gives that band divided by one less the marginal rate), and a published null exactly (a hand value of null passes only on the engine's null, never on a missing figure); the file carries one `it()` title, which is what the engine's coverage index publishes under `walkthroughs` |
| Evidence | `DOCS/operations/walkthroughs/<id>.json` | the rows with the engine's figures beside the hand values, the reading notes and the plan as built; written by `pnpm walkthroughs:export` in `packages/planner-ui` and held fresh by `walkthroughEvidence.test.ts`; format version 2 (version 1 plus null figures). The site never reads these files live: it renders a snapshot committed in its own repository, pinned to one engine commit, and its refresh refuses a file whose format version it does not know before staging anything, so a new version reaches the page only with the site change that reads it |
...
| bracket-fill-roth | Bracket-fill Roth conversions (`buildBracketFillRoth.ts`) | 2026, 2029 (2027 and 2028 as bridges) | [bracket-fill-roth.md](bracket-fill-roth.md), part II for 2029 | [REVIEW-2026-09-22.md](REVIEW-2026-09-22.md), one section per year checked | `bracketFillRoth.walkthrough.ts`, `bracket-fill-roth.test.ts` |
