...
| `rules/` | `records/` (one typed frozen record per statutory rule, in per-domain modules), `taxRuleRegistry.ts` (the types, the spread-composed frozen registry, and the re-verification helpers), `describeRule.ts` (the fixture helper that requires a `produced` reading for an `approximated` record) and `describeRefusal.ts` (its sibling for the `outOfScope` records whose declared `outOfScope.shape` is `typedRefusal`, which pins the refusal instead of a computed value; the other shape, `inexpressibleInput`, owes `missingInputFacts` and no fixture), the conformance and quote-fidelity suites, `approximations/`, and `attestations/` (the per-top-level-directory coverage-attestation shards `coverageAttestations.ts` composes, the same way `records/` composes into `taxRuleRegistry.ts`). The calculation catalog lives here too: `calculationRegistry.ts` (its types and the spread-composed frozen `CALCULATION_REGISTRY`) with its per-group record slices under `calculations/`, `describeCalculation.ts` (the evidence-fixture helper that binds a `*.evidence.test.ts` block to a record's worksheet and mutation receipt), and the generated output census, `outputFamilies.ts` and `outputFieldCoverage.ts`, regenerated from the `census/` JSON copies by `scripts/import-output-census.mjs` and pinned to them by `outputCensus.freshness.test.ts` |
...
| `rmd/` | `rmd.ts` |
| `socialSecurity/` | Pure SS math consumed by the ledger: `nra`, `benefitFactor`, `claimFactor`, `piaFromEarnings`, `ssaWageData`, `maritalBenefits`, `currentSpouseBenefit`, `survivorBenefit`, `familyMaximum`, `disability` |
