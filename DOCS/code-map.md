...
| `learn/` | Learning Center: pages, `learningRegistry.ts` (types + selectors), `articleIndex.ts` (metadata for all 138 articles, statically imported), `articleBodies.ts` (per-article `import()` map), `glossary.ts`, `components/`, 138 articles in `content/` (bodies only) |
