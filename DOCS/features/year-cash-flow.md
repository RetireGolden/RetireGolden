...
the economic ledger. If the post-HECM shortage exceeds every requested spending line, the residual is attributed, in order, to the settled-tax line and then to penalty lines (pro rata by remaining requested amount), so the cash identity still closes on a year whose tax liability outruns its cash. Contribution lines never carry that residual: their account credits are committed before the funding solve, so their dollars physically moved.
