...
`simulatePlan` will publish the detail only when the `SimulateOptions.captureAnnualCashFlow` option is `true`. That option and the capture logic land together in the capture workstream; this document and the reporting types are the contract they implement. The option defaults off and is intended only for the live deterministic Results projection. Monte Carlo,
...
| `incomes.pension` and its public/private/ordinary subsets | `spendableSource` / `pension` | Pension `accountId`, plus the actual payee's `personId` - the living survivor, not the deceased owner, when `survivorPct` continues the benefit | The paid amount is one source and the recipient identity is required, exactly as on the annuity row; the producing site selects the living payee. Public/private and ordinary subsets are `nonCashTaxCharacter`, not more cash. |
...
| `incomes.recurring`, its `ordinaryIncome` subset | `spendableSource` / `recurringIncome` | `incomeStreamId`; no person is inferred | Full stream amount is cash. Ordinary treatment is metadata only. Nothing is published once the household has died - no post-household cash-flow path (domain rules §19), and the stream carries no person link, so the gate is the household's rather than any one person's. |
| `incomes.oneTime`, `ordinaryIncome`, `oneTimeGains` | `spendableSource` / `oneTimeIncome` | `incomeStreamId`; no person is inferred | Full amount as elected is one cash source: `inflationAdjusted` streams publish the configured amount grown to the event year, the rest publish it as configured. Plans migrated from schema v4 carry the election off, which is what they already projected. Ordinary and gain portions are `nonCashTaxCharacter`. Nothing is published once the household has died, on the same §19 rule as the recurring row; this row was the ledger's one exception to it and no longer is. |
| `taxableInterest`, ordinary dividends, `qualifiedDividends`, `taxableYield`, `distributedYieldByAccountId` | `spendableSource` / `taxableAccountYield` when distributed | Taxable `accountId` | Publish one physical distributed-yield amount per account containing the TAXABLE gross only - `distributedYieldByAccountId.gross` also includes the account's tax-exempt interest, which the next row publishes separately, so emitting the map's gross here would count the exempt dollars twice and break the source total against `incomes.total`. Interest, ordinary-dividend, and qualified-dividend figures characterize that amount and are not additional sources. Reinvested accounts are handled only in the transfer table. |
...
| `debtService` | `fundedUse` + `unfundedUse` / `debtService` | Debt `accountId` where captured | Principal and interest remain one cash use. Debt-balance reduction is not a transfer line, and the ledger commits it for the FULL scheduled payment before funding resolves - so an unfunded portion on this line discloses cash the household never produced while the balance mutation stands. That asymmetry is an explicit ledger limitation this contract reports truthfully rather than hides. No owner is inferred. |
...
|------------------------|----------------|-------------------------------|--------------------------------------------|
| Allowed employee `contributions`; contribution runtime credits | `transfer` / `employeeContribution` after the contribution use | `householdCash` → destination `accountId`; owner where present | Debit and credit equal the committed credited amount; when the last-resort residual attribution reaches a contribution line, that credit exceeds the funded portion of the use and the difference is the use line's unfunded remainder. `sameDollarLaterStage` lineage points to that use when the credit equals the funded amount; in the committed-credit edge the relationship is `committedCreditBeyondFunding`, whose amounts deliberately differ by exactly the unfunded remainder. Never sum the two views. |
...
| `qcdIncomeOffset`, `qcdNonQualifiedOrdinaryIncome` | `nonCashTaxCharacter` on the corresponding QCD transfer | Same donor/action/account evidence as the gift | Exclusion or includible excess changes tax character, never the physical gift amount. The transfer kind names the physical IRA-to-charity channel only; a gift's qualified portion is exactly the `qcdIncomeExclusion` metadata, and dollars beyond it (an over-limit or post-70 1/2-offset excess) are non-qualified ordinary income that a consumer must never label as QCD dollars. |
...
At the engine's native floating-point precision, with the applied tolerances published in the
