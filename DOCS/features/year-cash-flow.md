...
| `incomes.oneTime`, `ordinaryIncome`, `oneTimeGains` | `spendableSource` / `oneTimeIncome` | `incomeStreamId`; no person is inferred | Full configured amount is one cash source, in dollars of its own event year - a one-time amount carries no inflation election and is never inflated, unlike `expenses.oneTimeGoals`. Ordinary and gain portions are `nonCashTaxCharacter`. Nothing is published once the household has died, on the same §19 rule as the recurring row; this row was the ledger's one exception to it and no longer is. |
