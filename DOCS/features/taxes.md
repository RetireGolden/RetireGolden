...
- **QCD:** a modeled household QCD's qualified portion is excluded from ordinary income, and the gift counts toward an RMD when one is due. It
  ...
  ending on or after 70½ counts, a pre-start named QCD that makes already-taken reductions unprovable
  fails the exclusion closed, and the published `qcd` total stays the gross gift. A named `qcd`
...
Cash, taxable, and equity-compensation ordinary withdrawals carry typed `notApplicable` /
`nonRetirementSource` penalty coverage rather than a computed section 72(t) amount: that additional tax reaches
an amount received from a qualified retirement plan
([IRC §72(t)(1)](https://uscode.house.gov/view.xhtml?req=granuleid:USC-prelim-title26-section72&num=0&edition=prelim);
`irc-72-t-1-qualified-retirement-plan-scope`). See also the domain-rules-reference non-retirement penalty scope
bullet listing all three sources.
...
proven-zero numerator the executor states the whole gross as taxable
(`irc-408A-d-3-A-i-zero-basis-conversion-includible`). At a positive one it commits the dollars and
...
(`hi-hrs-235-2-4-a-2-f-2026-standard-deduction`, [HI.md](../domain/state-tax-research/HI.md); the
§235-2.4(a)(2)(G) phase from 2028 is not carried), Rhode Island ADV 2025-22 TY2026 deduction and schedule
(`ri-dot-adv-2025-22-2026-deduction-and-rate-schedule`, [RI.md](../domain/state-tax-research/RI.md); re-read
ADV each autumn), and Utah S.B. 60 / §59-10-104 4.45% (`ut-code-59-10-104-2026-individual-rate`,
[UT.md](../domain/state-tax-research/UT.md)). State-only plan fixtures pin the modeled subtotal; whole-return
...
- **Inherited IRA schedules.** When beneficiary facts classify a regime, `projection/simulate.ts` executes
