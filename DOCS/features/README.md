...
qualified portion of the gift is deemed pre-tax, so it leaves income and returns no basis, while any portion
beyond a donor's aggregate includible amount is not qualified and stays on Form 8606 line 7 (2026 limit $111k,
...
annual ledger data, and optimizer recommendation evidence when available. Report data is assembled by the
edition-neutral [`report/reportModel.ts`](../../packages/planner-ui/src/report/reportModel.ts) (`buildReportModel` —
stable block ids, deterministic JSON serialization, CSV table export; published as the
`@retiregolden/planner-ui/report-model` subpath), which the HTML builder renders; golden HTML and
report-model fixtures for the reference cases live in
...
detail CSV from the captured engine `YearResult.cashFlow`; it never recomputes money math. The selector and
...
A one-page topology view at `/plan/:id/household-map` (Explore rail): people, income sources,
...
print styles, and an explicit "Not in the model" panel for relationships the schema cannot express
(dependents, trusts, named beneficiaries, debt collateral). Full detail:
