...
```
 ...
 ❯ src/montecarlo/marketModels.evidence.test.ts (16 tests | 1 failed) 9ms
   ❯ market-model-student-t-draw — Named student-t return shock: normal with a 5% tail-multiplier mixture (3)
     × df 3, u 0.01, z 1: shock 42 and inflation 0 4ms
...
+ false
 ...
 ❯ src/montecarlo/marketModels.evidence.test.ts:588:9
    586|         withinTolerance(path.returnShockPct[0]!, expectedShocks[2]!, e…
    587|         `returnShockPct ${path.returnShockPct[0]} is not within ${JSON…
    588|       ).toBe(true)
       |         ^
    589|       expect(
    590|         withinTolerance(path.inflationPct[0]!, expectedInflation[2]!, …
