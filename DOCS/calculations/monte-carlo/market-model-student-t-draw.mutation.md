...
+returnShockPct[i] = sigma * z * 100 + 12
```

Adds the configured volatility as a mean, the worksheet's first wrong reading (shock 12 rather than 0).
...
```
 ...
 ❯ src/montecarlo/marketModels.evidence.test.ts (13 tests | 5 failed) 9ms
   ...
   ❯ market-model-reversed-history — Reversed-history window replay (1)
     × reverses [2000, 2001, 2002] to [2002, 2001, 2000] 0ms
   ❯ market-model-student-t-draw — Centered Student-t return shock scaled to target volatility (1)
     × a zero t draw at volatility 12 is shock 0 0ms

 Test Files  1 failed (1)
      Tests  5 failed | 8 passed (13)

(node:34224) Warning: The 'NO_COLOR' env is ignored due to the 'FORCE_COLOR' env being set.
...
⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯[4/5]⎯
 ...
 FAIL  src/montecarlo/marketModels.evidence.test.ts > market-model-student-t-draw — Centered Student-t return shock scaled to target volatility > a zero t draw at volatility 12 is shock 0
AssertionError: returnShockPct 12 is not within {"abs":0} of the worksheet's 0: expected false to be true // Object.is equality
...
+ false
 ...
 ❯ src/montecarlo/marketModels.evidence.test.ts:471:9
    469|         withinTolerance(shock, expected, example.tolerance),
    470|         `returnShockPct ${shock} is not within ${JSON.stringify(exampl…
    471|       ).toBe(true)
       |         ^
    472|     })
    473|   },
