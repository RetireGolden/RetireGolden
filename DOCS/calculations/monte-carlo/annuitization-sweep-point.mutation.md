...
`git checkout -- packages/engine/src/decisions/annuitization.ts` restored the file, and `git diff --quiet -- packages/engine/src/decisions/annuitization.ts` then exited 0, confirming no production change remained. Re-ran the named command after restoration: the named file passed again (2 passed, exit 0).
