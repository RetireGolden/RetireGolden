...
Kind: formula. `allocation/assetClasses.ts#choleskyDecompose` returns a lower-triangular factor `L` with `LL^T` equal to the correlation matrix. The matrix must be positive definite: when a diagonal pivot `A_ii - sum_{k<i} L_ik^2` is not a positive number, no real factor exists and the matrix is refused with a RangeError (it used to be raised to `1e-12` without a word, which returned the factor of a different matrix).
