...
```
 ...
 ❯ src/montecarlo/marketModels.evidence.test.ts (13 tests | 5 failed) 9ms
   ...
   ❯ market-model-regime-switch — Two-state bull/bear Markov return shock (1)
     × bull start, U = 0.9 >= 0.05, zero vol: shock +4 0ms
...
(node:42604) Warning: The 'NO_COLOR' env is ignored due to the 'FORCE_COLOR' env being set.
(Use `node --trace-warnings ...` to show where the warning was created)
(node:42852) Warning: The 'NO_COLOR' env is ignored due to the 'FORCE_COLOR' env being set.
(Use `node --trace-warnings ...` to show where the warning was created)
...
+ -4
 ...
 ❯ src/montecarlo/marketModels.evidence.test.ts:367:38
    365|         model.generatePath(scriptedRng({ uniforms: [0.6, example.input…
    366|       )
    367|       expect(path.returnShockPct[0]).toBe(example.expected.returnShock…
       |                                      ^
    368|       expect(example.inputs.currentState).toBe(example.expected.state)
    369|     })
