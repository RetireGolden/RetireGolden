...
Derived by: codex (gpt-5.6-sol), 2026-09-18, from the signatures-and-comments extract (with the 2026-09-18 doc-comment corrections) and the orchestrator's contract statements, without executing the engine or reading any implementation body. Reviewed by: nobody approving yet. cursor (composer-2.5) rejected the worksheet three times on its own arithmetic: cases (b) and (c) on 2026-09-18 on hand-shifted bit strings (addenda 1 and 2), and case (b) on 2026-09-21 on a wrong product in a decimal division (addendum 4, which approves cases (a) and (c)); the orchestrator checked each disputed step by hand and by a script over the stated recurrence (REVIEW-2026-09-18.md in this directory), so the worksheet stands, but those checks are not the independent recomputation the record's provenance reports. The record is marked unreviewed; whether a script-executing lane may review bit-arithmetic worksheets is a queued decision (D-BIT-ARITHMETIC-REVIEWER).
