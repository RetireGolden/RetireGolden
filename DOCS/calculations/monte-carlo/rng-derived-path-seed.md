...
Derived by: codex (gpt-5.6-sol), 2026-09-18, from the signatures-and-comments extract (with the 2026-09-18 doc-comment corrections) and the orchestrator's contract statements, without executing the engine or reading any implementation body. Reviewed by: nobody approving yet. cursor (composer-2.5), 2026-09-18, rejected cases (b) and (c) on its own `h2 >>> 15` operands; the orchestrator verified the disputed folds by hand and by a script over the stated recurrence (REVIEW-2026-09-18.md in this directory: addenda 1 and 2 and the orchestrator verification), so the worksheet stands, but the orchestrator's check is not the independent recomputation the record's provenance reports. The record is marked unreviewed until an approving recomputation from an independent lane lands.
