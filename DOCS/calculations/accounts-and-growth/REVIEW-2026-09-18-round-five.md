...
# Re-check, 2026-09-18 (investable total after the comment correction)

The first derivation and its approval both followed the YearResult.investableTotal comment, which omitted equity-compensation accounts; a read-only research pass found that the engine sums them (the balances rows are cash, taxable, equityComp, traditional, roth and hsa). The comment was corrected, Codex Sol re-derived the worksheet (592,000, equity compensation a member), and the same reviewer recomputed it on the corrected extract and approved. Verbatim output follows.

---

## Independent review: `accounts-investable-total-annual`

### 1. Recomputation from Inputs and corrected comment member list

**Authoritative member list** (from `YearResult.investableTotal` comment in the extract):

> cash, taxable, equity compensation, traditional, Roth and HSA accounts, plus unassigned cash

**Members** (included in the sum):

| Input row | Value |
|---|---:|
| Cash accounts | $15,000 |
| Taxable accounts | $120,000 |
| Traditional accounts | $300,000 |
| Roth accounts | $90,000 |
| HSA accounts | $25,000 |
| Unassigned cash | $2,000 |
| Equity compensation | $40,000 |

**Discriminators** (present in Inputs but excluded from `investableTotal` per the comment; they test wrong readings):

| Input row | Value | Role |
|---|---:|---|
| Insurance cash value | $12,000 | Excluded; counted in `netWorth`, not `investableTotal` |
| TIPS-ladder value | $30,000 | Excluded; counted in `netWorth`, not `investableTotal` |

Property and debts are also named exclusions in the comment and Claim but do not appear as Input rows in this fixture.

**Recomputation:**

$15,000 + $120,000 + $300,000 + $90,000 + $25,000 + $2,000 + $40,000 = **$592,000**

### 2. Match with Expected

**Yes.** Expected `investableTotal = $592,000` matches the recomputed sum.

### 3. Wrong readings

| Wrong reading | Stated value | Recomputed | Correct? |
|---|---:|---:|---|
| Omit equity compensation | $552,000 | $592,000 − $40,000 = $552,000 | **Yes** |
| Add insurance cash value | $604,000 | $592,000 + $12,000 = $604,000 | **Yes** |
| Add ladder value | $622,000 | $592,000 + $30,000 = $622,000 | **Yes** |

Each stated discriminator value is arithmetically correct.

### 4. Claim vs corrected comment

**Member list — match.** Claim and comment both list: cash, taxable, equity compensation, traditional, Roth, HSA, plus unassigned cash.

**Exclusions — match.** Both exclude property, debts, insurance cash value, and the TIPS ladder, and both state that `netWorth` adds those channels (`investableTotal + property + insuranceCashValue + ladderValue − debt − HECM loans` in the `netWorth` comment).

The re-derivation correctly incorporates equity compensation, which the prior comment omitted.

### 5. Verdict

**Approve.** The seven-member sum recomputes to $592,000, all three wrong readings hold, and the Claim aligns with the corrected `investableTotal` and `netWorth` comments.
