...
Exact published `inheritedDistribution = $11,000`. Exact `inheritedTraditionalDistribution = $8,600`, the ordinary income the decision settled on; also retain the Roth gross forced amount `$3,000` and characterized taxable slice `$600` as evidence of how that figure is composed. Fixture tolerance: exact, because these are additions of whole-dollar row amounts.
...
- Following the field comment's earlier wording, which excluded Roth dollars outright, would report `$8,000`; the comment now states the composition that reports `$8,600`, which D-INHERITED-ROTH-SLICE settled as the intended meaning.
...
Amended 2026-09-25 by claude, the implementer of decision D-INHERITED-ROTH-SLICE: the decision settled the meaning as the ordinary income from inherited accounts, the composition this worksheet already derives, so the Claim, Justification, Expected and fourth wrong reading record it as settled rather than queued. No input, arithmetic or expected value changed.
