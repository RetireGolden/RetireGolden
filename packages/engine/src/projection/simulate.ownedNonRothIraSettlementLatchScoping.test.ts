...
describe('owned-IRA settlement latch scoping', () => {
  ...
  it('leaves a co-owner’s basis chain uninterrupted through the other’s annuity year', () => {
    // The refusal that fires here is the application-chain arm, which carries
    // the owner on its context, so it is p1's year that is disqualified and not
    // the household's. `publishableOwners` is still all-or-nothing -- the joined
    // artifact is withheld for that year because p1 appears in it -- so what
    // this asserts is that the ECONOMICS survive for p2 and that publication
    // resumes the next year for both.
    const couple = (withAnnuity: boolean): Plan => {
      ...
      plan.id = `latch-scoping-couple-one-owner-annuity-${String(withAnnuity)}`
      plan.accounts = [
        ira('ira-p1', 100_000, 20_000, 'p1'),
        ira('ira-p2', 100_000, 20_000, 'p2'),
        ...(withAnnuity
    ...
    // p2's ECONOMICS survive the year their co-owner's purchase blocked, and
    // survive it exactly: their TAX_YEAR + 2 opening basis, their carryforward
    // out of it, and their balance are all cent-for-cent what the same household
    // produces with no annuity in it at all. The fallback year priced p2's pool
    // to the same figure the settlement would have.
    expect(openingBasisCents(years, 'p2')[2]).toBe(1_831_961)
    expect(openingBasisCents(control, 'p2')[2]).toBe(1_831_961)
    expect(nextOpeningBasisCents(years, 2, 'p2'))
      .toBe(nextOpeningBasisCents(control, 2, 'p2'))
    expect(years[2]!.balances['ira-p2'])
      .toBe(control[2]!.balances['ira-p2'])
    // p1's own figure legitimately diverges -- 5,000 dollars left their pool --
    // and they are settling again all the same, so the year-scoped disposition
    // did not quietly become a per-owner permanent one.
    expect(openingBasisCents(years, 'p1')[2]).toBe(1_827_353)
  })
