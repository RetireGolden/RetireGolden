...
describe('annualContributionsAndEmployerMatch — positional planning', () => {
  it('keeps duplicate-id requests positional with exact source identity and raw owner effects', () => {
    const rows = [
      ...
      balance(account('roth', 'duplicate', 4, {
        ownerPersonId: 'p2',
      })),
    ]
    const result = call(rows, {
      ...
      aliveCount: 2,
      peopleCount: 2,
      wagesByPerson: new Map([['p1', 4], ['p2', 2]]),
      resolveOwnerState: (owner) => ({
        alive: true,
        ageAttained: owner === 'p1' ? 40 : 41,
      }),
      resolveOwnerBirthYear: (owner) => owner === 'p1' ? 1986 : 1985,
      resolveOwnerDob: (owner) => owner === 'p1'
        ? '1986-01-01'
        : '1985-01-01',
    })
