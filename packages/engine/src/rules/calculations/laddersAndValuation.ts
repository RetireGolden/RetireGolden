...
export const laddersAndValuationRecords = {
  ...
  'income-tips-ladder-and-ladder-value-annual': {
    ...
    purpose: 'The cash a TIPS income-floor ladder pays in a year and the face still riding in net worth at its end.',
    kind: 'composition',
    outputs: ['income-tips-ladder-annual', 'ladder-value-annual'],
    statement:
      'At offset = year − anchor year, projection/internal/tipsLadderAnnualCashFlow.ts#tipsLadderAnnualCashFlows publishes, per ladder: cash = (coupons + maturing principal) x the ladder\'s funding scale x the cumulative inflation factor from the start year, where coupons sum face x couponRatePct / 100 over every rung with maturityOffset >= offset and maturing principal is the face of the rungs maturing at exactly that offset; and ladderValue = unmatured face (rungs with maturityOffset > offset) x the same scale and factor. In the purchase year (offset 0) cash is 0 and the value is the whole face at that year\'s factor; with nobody alive the remaining face is frozen at the last living year\'s offset. YearIncomes.tipsLadder and YearResult.ladderValue are the sums of these rows over the plan\'s ladders. Units: nominal dollars. Rounding: none.',
    formula: {
      expression: 'cash = (Σ_{m >= k} F_m c_m + Σ_{m = k} F_m) s f_k; value = (Σ_{m > k} F_m) s f_k; cash = 0 at k = 0',
      variables: [
        { symbol: 'k', meaning: 'Offset from the anchor year (purchase year, or the year before the start when already owned)', unit: 'years', domain: 'integer >= 0' },
        { symbol: 'F_m, c_m', meaning: 'Real face and coupon rate of the rung maturing at offset m', unit: 'usd / fraction', domain: 'F_m >= 0, 0 <= c_m' },
        { symbol: 's', meaning: 'Funding scale: 1, or the funded fraction when the purchase-year account could not cover the quoted cost', unit: '1', domain: '0 <= s <= 1' },
        { symbol: 'f_k', meaning: 'Cumulative general-inflation factor from the start year to this year', unit: '1', domain: 'positive' },
      ],
      timing: 'annual; one row per ladder, never summed inside the phase',
      rounding: 'none',
    },
    justification: {
      kind: 'derivation',
