...
export const monteCarloRecords = {
  ...
  'market-model-stationary-bootstrap': {
    ...
    formula: {
      ...
      variables: [
        { symbol: 'L', meaning: 'Mean block length after the floor of 2', unit: 'years', domain: '>= 2; worksheet 5' },
    ...
    limits: [
      ...
      'L = max(2, meanBlockLength ?? 5); a requested mean below 2 is raised to 2',
      'Draw order per block is nextInt(n) then next(); a new pair is drawn only when remaining reaches 0',
      'The || 1 clamp rejects an empty block: at U = 0.01 and L = 5, floor(−ln(0.99) · 5) = 0 would otherwise be length 0',
    ],
    implementedBy: ['packages/engine/src/montecarlo/marketModels.ts'],
