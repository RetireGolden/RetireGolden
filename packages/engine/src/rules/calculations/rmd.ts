...
export const rmdRecords = {
  'qcd-income-offset-qualified-slice': {
    ...
    limits: [
      'Disclosed discrepancy on the two character components. On the worksheet\'s own inputs (60,000 gift, 50,000 from RMD, 40,000 aggregate includible, 5,000 remaining offset) the engine publishes incomeOffsetDelta 30,000 where the worksheet expects a 35,000 income offset, and nonQualifiedOrdinaryIncomeDelta 5,000 where the worksheet expects a 10,000 beyond-RMD ordinary delta. The engine charges every non-qualified dollar to the from-RMD portion first, so qualifiedFromRmd is 50,000 - 20,000 = 30,000 and the exclusion cannot exceed it; the worksheet, and the YearResult.qcd comment\'s shorthand min(gift from RMD, aggregate includible amount), allocate the qualified slice to the from-RMD portion first instead. The two readings agree on the gross gift, the qualified slice, the total non-qualified gift and the resulting 25,000 ordinary inclusion, and differ only in how that inclusion is split between the offset and the beyond-RMD delta. The evidence keeps the worksheet\'s expectations for the two components and fails.',
