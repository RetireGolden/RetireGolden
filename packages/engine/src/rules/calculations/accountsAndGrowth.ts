...
export const accountsAndGrowthRecords = {
  ...
  'accounts-balance-per-account-annual': {
    ...
    limits: [
      ...
      'Until decision D-CASH-PROPERTY-ALIAS (2026-09-25, extended the next day) the plan checks accepted several pairs on a single id: a cash account and a property, a property and a debt, two properties, two debts, an account and a permanent-life policy, and two policies. The later value then overwrote the earlier one in this map and in every figure read from it (a $300,000 home published as cash), and two properties or two life policies under one id counted only one of them in net worth. The checks now refuse every such pair, and a stored plan that has one gets new ids for the colliding rows when it is loaded (an investable account keeps the id), so no plan reaches the snapshot with one',
    ],
    implementedBy: [
      'packages/engine/src/projection/internal/annualSnapshot.ts',
