...
export const accountsAndGrowthRecords = {
  ...
  'accounts-balance-per-account-annual': {
    ...
    limits: [
      ...
      'The collision case is asserted by calling annualSnapshot directly with four channels on one id, because no valid plan can reach the snapshot with two channels on one id: the plan checks refuse every pair of rows that would publish a value under a shared id (model/sharedIdCollisions.ts). The overwrite order is a property of the snapshot, not of any plan',
      'The map holds no HECM line: HECM debt is published separately and only enters net worth under its non-recourse cap',
      'Until decision D-CASH-PROPERTY-ALIAS (2026-09-25, extended the next day) the plan checks accepted several pairs on a single id: a cash account and a property, a property and a debt, two properties, two debts, an account and a permanent-life policy, and two permanent-life policies. The later value then overwrote the earlier one in this map and in every figure read from it (a $300,000 home published as cash), and two properties, two debts or two permanent-life policies under one id counted only one of them in net worth. The checks now refuse every such pair, and a stored plan that has one gets new ids for the colliding rows when it is loaded (an investable account keeps the id), so no plan reaches the snapshot with one',
    ],
    implementedBy: [
      'packages/engine/src/projection/internal/annualSnapshot.ts',
