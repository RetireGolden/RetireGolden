...
export const spendingAndWithdrawalsRecords = {
  ...
  'spending-base-annual': {
    ...
    limits: [
      ...
      'The worksheet\'s year is not a state annualGuardrailFundingPlan produces: a cutting year funds no upside at all, because the upside budget is max(0, multiplier - 1) x the guardrail step basis, which is 0 at a multiplier of 0.75. A year with a 0.75 factor AND $4,000 of funded ideal and $1,500 of funded excess therefore cannot arise from that phase. The min(1, factor) cap is asserted at annualGuardrailFundingPlan with the worksheet\'s own $24,000 target layer and 0.75 multiplier, and the resulting $18,000 is handed to annualExpenseSummary with the worksheet\'s remaining layers',
      'The $8,000 one-time goal is passed to the same seam call and asserted to be absent from baseSpending while present in expenses.total, which is the worksheet\'s second wrong reading',
    ],
    implementedBy: [
      'packages/engine/src/projection/internal/annualExpenseSummary.ts',
