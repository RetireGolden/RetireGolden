...
export const modelAttestations: Readonly<Record<string, CoverageAttestation>> = Object.freeze({
  ...
  'model/sharedIdCollisions.ts': Object.freeze({ status: 'rule-free', sweptOn: '2026-09-26', note: 'Which plan rows would publish a value under a shared id (accounts, properties, debts, insurance policies) and which row keeps the id; plan-integrity bookkeeping read by the plan checks and the load repair, with no statutory amount, tax or entitlement.' }),
})
