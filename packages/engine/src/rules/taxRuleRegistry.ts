...
const registry = {
  ...
  'irc-408-d-2-C-projection-pro-rata-measurement-instant': {
    ...
    statement:
      'Section 408(d)(2)(C) fixes the section 72 contract value at the CLOSE of the calendar year and then adds the year’s distributions back to it, which is what Form 8606 line 6 asks for — the December 31 value of the traditional IRAs, after a full year of investment return on whatever the account retained. The engine takes that instant wherever the owned-non-Roth-IRA annual settlement governs the year: the settlement builds its denominator as the post-growth December 31 pool balance plus lines 7 and 8, and the attempt driver re-runs the whole annual pass until the characters it assumed are the ones the run produced. That now includes a year with an aggregate charitable gift routed out of a required distribution, which the settlement refused until 2026-08-07. The LEGACY FALLBACK ledger does not, and it governs every year the settlement produces no usable characters for: it measures the aggregated balance immediately after contributions and immediately BEFORE the year’s first distribution. Those two instants agree on the dollars that were distributed, because the ledger credits growth after distributions, and they DISAGREE by exactly the growth earned on the balance that stayed in the account. The fallback measure is therefore year-end-BEFORE-growth plus distributions, not line 6 plus distributions, and it is invariant to the return assumption where the statutory measure is not.',
    classification: 'approximated',
    contraryReading: null,
    errorDirection: 'bothDirections',
    conventionRationale:
      'SCOPE, WHICH IS THE FIRST THING THIS RECORD GOT WRONG AND THE THING THAT HAS MOVED SINCE. The departure lives in the legacy fallback ledger, and a year reaches that ledger only when the owned-non-Roth-IRA annual settlement produces no usable Form 8606 characters for it. Every shape the settlement covers already takes the statutory instant, including the shapes where the year’s distributions depend on the very fraction being computed: a bracket-targeted need-based withdrawal settles, because the attempt driver iterates the whole annual pass rather than trying to solve the fraction in place. THE SHAPE STILL REACHABLE ON 2026-08-07 IS ONE, and it is not a charitable gift. An owned-IRA-funded annuity purchase refuses with annuityStageRequired — the premium leaves the captured owned-IRA pool for a contract the replay does not carry — and it is an ordinary valid plan. The produced fixture is built on the annuity one, and it holds the pool’s COMPOSITION fixed across its two readings so that the only thing they disagree about is the instant: whether the premium should have left the section 72 pool at all is a different question with its own refusal. GONE FROM THIS SCOPE, and the reason the record was rewritten again: an aggregate strategies.qcdAnnual gift routed out of a required distribution used to mint a nonmoving overlay that carried no owner and no source account, a household scalar could not say whose line-7 gross the gift shrinks, and the source series refused the year with qcdStageRequired. The overlay now carries the per-owner 408(d)(8)(D) attribution the annual ledger settles when it sizes the gift — the routed gross and the qualified part of it — and the source series carves that qualified amount out of the owner’s required-distribution line-7 gross in mutation order, reproducing the ledger’s own commit rather than deriving a rival one. A gift year therefore settles, at the December 31 measure, and it is a settled-path fixture that now pins it. A NAMED gift never reached the fallback: it is modelled as a distribution beyond the requirement, moves under its own occurrence, and settles. DIRECTION OF ERROR: the sign is the sign of the year’s return on the retained balance. In a gain year the statutory denominator is larger, the basis fraction smaller, and the fallback returns MORE basis than the form does — understating that year’s income and spending basis that later years then do not have. In a loss year the denominator shrinks below the fallback’s, the fraction rises, and the fallback returns LESS basis, overstating income. Neither is a timing wash within the year and the two do not cancel across a lifetime unless returns do. MAGNITUDE: the departure is (line 7 + line 8) × growth-on-the-retained-balance ÷ denominator, so it is negligible on a household whose only IRA activity is a required distribution and material on a conversion year, where line 8 can be many times line 7. PINNED ON BOTH SIDES OF THE BOUNDARY, by three fixtures rather than one. The fallback: a 76-year-old with a $1,000,000 IRA carrying $200,000 of basis, a $42,194.09 required distribution, a $100,000 Roth conversion and a $50,000 annuity premium paid out of the same IRA reports ordinary income of $112,258.49 at a 0 percent return, at a 5 percent return, and at a negative 5 percent return alike; at 5 percent the form computes line 9 as $990,390.30 and ordinary income as $113,479.33 — $1,220.84 more than the fallback reports. The settled path, twice: the same household without the premium reports $113,755.27 at a 0 percent return and $114,924.86 at a 5 percent one, and the same household with a $40,000 aggregate charitable gift instead reports $80,903.66 and $81,814.18 — the pair the fallback used to report identically at every return, which is what makes the second settled fixture the flipped pin rather than a new one. NOT CORRECTED HERE, and the reason has not changed. Giving the fallback the statutory denominator means giving it the same multi-pass fixed point the settlement already runs, because the denominator needs the year’s total distributions and those are decided after the fraction is needed and depend on it — there is no closed form available at distribution time. The remedy runs the other way, one refusal at a time: make the refusing arm source-allocatable so the settlement covers it, which removes the fallback from that shape instead of duplicating its fix. That is what closed the gift shape, it is what closed the exact-withdrawal shape, and it is what would close the annuity one. CORRECTION APPENDED 2026-08-07, LATER THE SAME DAY. The version above said two shapes were still reachable and named the second as a Plan-declared exact ordinary withdrawal sourced from an owned IRA. That one is now closed, and by the remedy this record prescribes rather than by a change to the fallback. The ordinary executor’s source scope is cash, equity compensation and taxable, so an owned-IRA allocation is refused there with withdrawal-source-type-unsupported: the declaration moves no dollars, changes no balance, and contributes nothing to line 7. The source series used to refuse the year on the declaration alone; it now binds to the executor’s own evidence for that action and refuses only where cents actually moved or where no evidence proves they did not. The record’s own fixture household plus a declared owned-IRA withdrawal settles, so that shape no longer reaches the fallback at all. NEITHER THE VERIFIER NOR THE AUTHOR COULD CONSTRUCT ANY VALID PLAN STILL RAISING exactActionStageRequired after that change — a declared owned-IRA withdrawal settles, and a balance large enough to escape the exact-cent ledger refuses earlier with sourceAmountInvalid — so the remaining producers of that kind are stated here as a fail-closed net of unknown reachability rather than as a live shape. They are deliberately not deleted. The annuity purchase is therefore the one shape this record’s departure is measured on, and the produced fixture is built on it. CORRECTION HISTORY. As first written on 2026-08-05 this record said "The annual ledger measures the denominator at a different instant" and "The engine denominator is invariant to the return assumption; the statutory one is not", and it named only simulate.ts and strategies/iraBasis.ts as implementing the rule. Both sentences generalized a one-shape observation to the whole engine. The observation itself was sound, but every other shape checked on 2026-08-07 moved with the return assumption and matched the closed-form statutory denominator to the cent, including a two-IRA household whose accounts grow at different rates, which is the case a single household-wide growth factor could not have produced. The record was re-scoped to the fallback and a settled-path fixture was added so the boundary was pinned rather than asserted. Re-scoped AGAIN later the same day: the version above claimed "the single refusal reachable today is qcdStageRequired", which was wrong twice over — the annuity and exact-action refusals were reachable when it was written, and the QCD one stopped being reachable when the gift became source-allocatable. This record exists in the first place because irc-408-d-8-D-projection-qcd-after-pro-rata cites 408(d)(2) INCLUDING the close-of-year sentence as authority for a denominator the fallback measures at a different instant, and an authority quoted for a measure some arm of the engine does not take must be registered rather than left implied.',
    jurisdiction: 'federal',
    authority: [{
      kind: 'statute',
    ...
    // wrong and no route to the two files that get it right.
    implementedBy: [
      'packages/engine/src/actions/ownedNonRothIraWithdrawalCharacter.ts',
      'packages/engine/src/internal/ownedNonRothIraRuntimeSourceSeries.ts',
      'packages/engine/src/projection/simulate.ts',
