...
const registry = {
  ...
  'irc-408-d-2-C-projection-pro-rata-measurement-instant': {
    ...
    statement:
      'Section 408(d)(2)(C) fixes the section 72 contract value at the CLOSE of the calendar year and then adds the year\u2019s distributions back to it, which is what Form 8606 line 6 asks for \u2014 the December 31 value of the traditional IRAs, after a full year of investment return on whatever the account retained. The engine takes that instant wherever the owned-non-Roth-IRA annual settlement governs the year: the settlement builds its denominator as the post-growth December 31 pool balance, plus the December 31 value of any annuity contract that pool bought, plus lines 7 and 8, and the attempt driver re-runs the whole annual pass until the characters it assumed are the ones the run produced. A LEGACY FALLBACK LEDGER still exists and still measures the aggregated balance immediately after contributions and immediately BEFORE the year\u2019s first distribution, which differs from the statutory instant by exactly the growth earned on the balance that stayed in the account. What has changed is that no valid Plan is known to reach it. Every refusal that used to route a year there has been closed by making the refusing arm source-allocatable, which is the remedy this record has prescribed since it was written; the fallback is retained as a fail-closed net rather than deleted, and its reachability is stated below as measured rather than assumed.',
    classification: 'settled',
    contraryReading: null,
    errorDirection: null,
    conventionRationale:
      'SETTLED ON 2026-08-07, AND WHAT THAT DOES AND DOES NOT CLAIM. It does not claim the fallback ledger was corrected. It claims the departure has no reachable shape: the legacy measure is still in the source, still measured before the year\u2019s first distribution, and would still be wrong for any year that reached it. Reclassifying rather than deleting is the disposition this codebase takes for a fail-closed net whose door is shut \u2014 the same one exactActionStageRequired already carried in this record \u2014 because a guard that cannot fire is not a defect a reader needs warned about, and an approximated entry that no fixture can produce is exactly the entry that rots into looking like diligence. REACHABILITY, AS MEASURED ON 2026-08-07 AND NOT AS ARGUED. Three refusals routed years to the fallback. The charitable one closed when the nonmoving overlay learned to carry its per-owner 408(d)(8)(D) attribution. The exact-action one closed when the source series bound itself to the executor\u2019s own evidence instead of to the mere presence of a declaration, and neither the verifier nor the author could then construct a valid Plan raising it. The annuity one closed here: a premium leaving the captured pool used to refuse the year because the contract it entered was something the replay did not carry, and the replay carries it now. What is left of that refusal fires only where the contract cannot be placed in ONE owner’s section 408(d)(2) aggregate, and every contract a valid Plan can express now can be: the aggregate is the FUNDING account’s owner’s, which an annuity’s own ownerPersonId does not decide, so a contract naming the other spouse or naming nobody stages like any other. A parse-time refusal of the cross-owner shape was written and withdrawn — it made a pinned v1 export unimportable, and a forward-migration guarantee outranks closing a shape that staging handles anyway; what the withdrawal leaves open is a 408(d)(1) question recorded on irc-408-d-1-ira-annuity-premium-is-not-a-distribution rather than here. The remaining producers are therefore the missing-occurrence and missing-credit pre-checks, both of which require a hand-mutated projection rather than a Plan. Eight shapes were probed for a surviving route \u2014 a cross-owner purchase named and unnamed, two purchases in one year, a purchase predating the projection, a contract that outlives its premium, balances at the top and bottom of the exact-cent ledger, and a premium the funding account could not pay \u2014 and every one of them settled. THE PROBE FOUND A REAL DEFECT WHILE LOOKING, which is recorded because it is the reason this claim is worth more than the last one. The last shape on that list did NOT settle at first: the contract-value channel was seeded by re-deriving the premium less the payments made before the replay\u2019s start year, which is right for a contract bought before the projection began and wrong for every contract whose funding account could not pay the whole premium, since the engine funds only what is spendable. A single-year replay of such a projection reconstructed a channel the projection never had. The opening now travels with the published source, bounded by the premium and required to be zero in any year at or before the purchase, and chained to the prior year\u2019s close across a multi-year series. PINNED FROM THE SETTLED SIDE ONLY, now that there is no other side. A 76-year-old with a 1,000,000 dollar IRA carrying 200,000 of basis, a 42,194.09 required distribution and a 100,000 dollar Roth conversion reports 113,755.27 at a 0 percent return and 114,924.86 at a 5 percent one; the same household with a 40,000 dollar aggregate charitable gift instead reports 80,903.66 and 81,814.18. The movement with the return IS the statutory instant: a denominator measured before the year\u2019s growth could not produce it. The same household with a 50,000 dollar annuity premium used to report 112,258.49 at every return alike, and that invariance was this record\u2019s produced fixture; it now moves with the return like every other shape. WHAT THE DEPARTURE WAS, kept so a reader who reopens the fallback knows what they are reopening. The sign is the sign of the year\u2019s return on the retained balance: in a gain year the statutory denominator is larger, the basis fraction smaller, and the fallback returns MORE basis than the form does, understating that year\u2019s income and spending basis later years then do not have; in a loss year the fallback returns less and overstates income. The magnitude is (line 7 + line 8) \u00d7 growth-on-the-retained-balance \u00f7 denominator, negligible on a household whose only IRA activity is a required distribution and material on a conversion year. Giving the fallback the statutory denominator still means giving it the same multi-pass fixed point the settlement runs, because the denominator needs the year\u2019s total distributions and those are decided after the fraction is needed and depend on it. CORRECTION HISTORY, unabridged, because this record has been wrong in public three times. As first written on 2026-08-05 it said "the annual ledger measures the denominator at a different instant" and "the engine denominator is invariant to the return assumption; the statutory one is not", and named only simulate.ts and strategies/iraBasis.ts as implementing the rule \u2014 both sentences generalized a one-shape observation to the whole engine, and every other shape checked on 2026-08-07 moved with the return and matched the closed-form statutory denominator to the cent. Re-scoped to the fallback, with a settled-path fixture added so the boundary was pinned rather than asserted. Re-scoped AGAIN later that day: it had claimed "the single refusal reachable today is qcdStageRequired", which was wrong twice over, since the annuity and exact-action refusals were reachable when it was written and the charitable one had stopped being. It then named the annuity purchase as the one shape its departure was measured on, and predicted its own end in terms \u2014 "if the annuity premium ever stops leaving the captured pool this shape settles, and then it is this suite, not only the record, that has to be revisited." It did, and it was.',
    jurisdiction: 'federal',
    authority: [{
      kind: 'statute',
