...
const registry = {
  ...
  'irc-408-d-2-C-projection-pro-rata-measurement-instant': {
    ...
    errorDirection: 'bothDirections',
    conventionRationale:
      'SCOPE, WHICH IS THE FIRST THING THIS RECORD GOT WRONG. The departure lives in the legacy fallback ledger, and a year reaches that ledger only when the owned-non-Roth-IRA annual settlement produces no usable Form 8606 characters for it. Every shape the settlement covers already takes the statutory instant, including the shapes where the year’s distributions depend on the very fraction being computed: a bracket-targeted need-based withdrawal settles, because the attempt driver iterates the whole annual pass rather than trying to solve the fraction in place. The single refusal reachable today is qcdStageRequired — an aggregate strategies.qcdAnnual gift routed out of a required distribution mints a nonmoving overlay that carries no owner and no source account, a household scalar cannot say whose line-7 gross the gift shrinks, so the source series refuses the year and the pass keeps the fallback figures it had already committed. A NAMED gift does not reach the fallback: it is modelled as a distribution beyond the requirement, moves under its own occurrence, and settles. DIRECTION OF ERROR: the sign is the sign of the year’s return on the retained balance. In a gain year the statutory denominator is larger, the basis fraction smaller, and the fallback returns MORE basis than the form does — understating that year’s income and spending basis that later years then do not have. In a loss year the denominator shrinks below the fallback’s, the fraction rises, and the fallback returns LESS basis, overstating income. Neither is a timing wash within the year and the two do not cancel across a lifetime unless returns do. MAGNITUDE: the departure is (line 7 + line 8) × growth-on-the-retained-balance ÷ denominator, so it is negligible on a household whose only IRA activity is a required distribution and material on a conversion year, where line 8 can be many times line 7. PINNED ON BOTH SIDES OF THE BOUNDARY, by two fixtures rather than one. The fallback: a 76-year-old with a $1,000,000 IRA carrying $200,000 of basis, a $42,194.09 required distribution, a $40,000 aggregate charitable gift and a $100,000 Roth conversion reports ordinary income of $80,903.66 at a 0 percent return, at a 5 percent return, and at a negative 5 percent return alike; at 5 percent the form computes line 6 as $900,696.20, line 9 as $1,002,890.30, a basis fraction of 0.1994236, and ordinary income of $81,814.18 — $910.52 more than the fallback reports. The settled path: the same household without the gift reports $113,755.27 at a 0 percent return and $114,924.86 at a 5 percent one, which is the December 31 measure and not the pre-distribution one. The second fixture is what stops this record being read again as a claim about the engine. NOT CORRECTED HERE, and the reason has changed. Giving the fallback the statutory denominator means giving it the same multi-pass fixed point the settlement already runs, because the denominator needs the year’s total distributions and those are decided after the fraction is needed and depend on it — there is no closed form available at distribution time. The tracked remedy runs the other way: make the aggregate gift arm source-allocatable so the settlement covers it, which removes the fallback from this shape instead of duplicating its fix. CORRECTION HISTORY. As first written on 2026-08-05 this record said "The annual ledger measures the denominator at a different instant" and "The engine denominator is invariant to the return assumption; the statutory one is not", and it named only simulate.ts and strategies/iraBasis.ts as implementing the rule. Both sentences generalized a one-shape observation to the whole engine. The observation itself was sound — the fixture holds an aggregate charitable gift, which is exactly the shape the settlement refuses — but every other shape checked on 2026-08-07 moved with the return assumption and matched the closed-form statutory denominator to the cent, including a two-IRA household whose accounts grow at different rates, which is the case a single household-wide growth factor could not have produced. The record was re-scoped to the fallback, the settlement chain was added to implementedBy, and the settled-path fixture was added so the boundary is pinned rather than asserted. This record exists in the first place because irc-408-d-8-D-projection-qcd-after-pro-rata cites 408(d)(2) INCLUDING the close-of-year sentence as authority for a denominator the fallback measures at a different instant, and an authority quoted for a measure some arm of the engine does not take must be registered rather than left implied.',
    jurisdiction: 'federal',
    authority: [{
      kind: 'statute',
