...
export const healthSavingsAccountRecords = {
  ...
  'irc-223-b-2-7-projection-coverage-proration-and-medicare': {
    ...
    statement:
      'The HSA limitation is monthly: 1/12 of the annual amount for each month the taxpayer is an eligible individual, at the self-only or family tier according to the coverage actually held on the first day of that month, and zero for the first month of entitlement to Medicare and every month after. Not modelled: coverage tier, monthly eligibility, and Medicare entitlement are source-side facts absent from the plan schema. The annual ledger substitutes household size for the HDHP election — a two-person household selects the family base, then paragraph (5) equal division halves that base across two MFJ spouses even when only one person holds an HSA, so a 2026 MFJ one-owner case yields $4,375 (half of $8,750) rather than the $4,400 self-only statute reading. It also applies a whole-year limit regardless of eligible months and never zeroes the limit when Medicare entitlement begins; those two shortcuts are permissive while the coverage shortcut can be restrictive, so net error runs both directions.',
    classification: 'approximated',
    contraryReading: null,
