...
export const contributionAndDeferralLimitRecords = {
  ...
  'irc-414-v-2-E-super-catch-up-window': {
    ...
    errorDirection: null,
    conventionRationale:
      'Section 414(v)(2)(C)(i) adjusts the adjusted dollar amounts applicable under (E), so the amount produced by the greater-of test — 11,250 dollars for a non-SIMPLE plan in 2026 — is the indexed amount; the 10,000 dollar operand is not a separate projected leg. The 2026 pack therefore carries 11,250, and the ages-60-through-63 branch sends that amount and cumulative limitGrowth to indexWithStatutoryRounding, which floors the cumulative increase to a multiple of 500 dollars. This preserves the statutory indexing-and-rounding shape but not its exact CPI input: limitGrowth is measured from the 2026 pack year rather than the statutory calendar quarter beginning July 1, 2024, and it does not consume future IRS-published annual limits, so future projected dollar amounts remain a planning approximation. Notice 2025-67 confirms the published 2026 amount; its remaining 11,250 while the ordinary catch-up rose from 7,500 to 8,000 does not distinguish a permanently fixed amount from an indexed amount whose increase did not clear a 500-dollar step. The age-55 HSA addition is registered separately because section 223(g) omits it from indexing entirely.',
    jurisdiction: 'federal',
    authority: [{
      kind: 'statute',
