...
export const southCentralStateRecords = {
  ...
  'ms-27-7-17-standard-deduction-unindexed': {
    ...
    statement:
      'Mississippi’s optional standard deduction is $4,600 for married individuals filing a joint or combined return and $2,300 for an individual who is not married. The pack carries those figures only for supported single filers and married-filing-jointly returns; head-of-family and married-filing-separately limbs are outside this record’s supported scope and remain unmodeled — see DOCS/domain/state-tax-research/MS.md for the full statutory schedule. This is non-exhaustive supported-scope coverage, not a claim that every §27-7-17(3)(b) amount is modeled. The statute states the quoted amounts as "for each calendar year thereafter" following 1998 and provides no indexation, so unlike Arkansas’s or North Dakota’s they do not move and cannot go stale — which is also why the pack entry carries no `standardDeductionConformity` tag: nothing in Mississippi law references IRC 63(c), and tagging it would move a frozen state figure with an annually rising federal one and hand the Mississippi base a federal age-65 addition Mississippi does not grant.',
    classification: 'settled',
    contraryReading: null,
    errorDirection: null,
    conventionRationale: null,
  ...
  'ms-27-7-21-personal-and-age-65-exemptions': {
    ...
    statement:
      'In addition to the standard or itemized deduction, a Mississippi resident subtracts a personal exemption of $6,000 if single or $12,000 jointly for married individuals living together, plus a further $1,500 for each of the taxpayer and spouse who has attained age 65 before the close of the taxable year. Form 80-105 runs them as their own line, below the deduction and above taxable income. The pack models no personal exemption anywhere; this record quotes and discloses only the supported single and joint personal-exemption amounts and the age-65 addition that the age-65 fixture exercises. Head-of-family, dependent, and blindness exemptions remain unmodeled and are documented only in DOCS/domain/state-tax-research/MS.md — not re-quoted here. Non-exhaustive supported-scope coverage: folding Mississippi’s exemptions into `standardDeduction` would make one state an unmarked exception to a fifty-one-state convention. The result is that a married couple both 65 or over is charged 4% on $15,000 Mississippi exempts — about $600 a year — and a single filer aged 65 or over on $7,500, about $300. Note which way this runs and against whom: it OVER-charges the modal Mississippi retiree, whose pension and Social Security are already outside the base and whose remaining income is investment income, while `ms-early-or-excess-distribution-not-exempt` UNDER-charges the early retiree. Mississippi is the state in this pack where the sign of the error flips with the household’s age, and the flip point sits where a planner’s most interesting decisions get made — so the two records must be read separately rather than netted into one direction.',
    classification: 'approximated',
    contraryReading: null,
    errorDirection: 'overstatesTax',
    conventionRationale: null,
