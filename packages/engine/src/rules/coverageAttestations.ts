...
export const COVERAGE_ATTESTATIONS: Readonly<Record<string, CoverageAttestation>> = Object.freeze({
  ...
  'projection/flatTax.ts': Object.freeze({ status: 'rule-free', sweptOn: '2026-08-30', note: 'deprecated re-export that keeps the published projection/flatTax subpath resolving; the flat-rate test double it forwards to lives at testing/flatTax.ts and carries the IRC 86 claim' }),
  ...
  'testing/flatTax.ts': Object.freeze({ status: 'partial', sweptOn: '2026-08-30', note: 'flat-rate test double whose only importers are test files, so the IRC 86 85 percent inclusion it applies never reaches a user-facing number; kept partial rather than rule-free so the claim stays visible if the file ever gains a production consumer' }),
