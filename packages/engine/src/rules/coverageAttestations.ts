...
export const COVERAGE_ATTESTATIONS: Readonly<Record<string, CoverageAttestation>> = Object.freeze({
  ...
  'projection/internal/wageIncomeStreams.ts': Object.freeze({ status: 'rule-free', sweptOn: '2026-08-31', note: 'income pass 1 — wages — moved out of simulatePlan. It decides which wage streams pay in a given year and how much; it computes no tax and encodes no statutory rule, since the stop-age gates and the real-raise and inflation factors are all plan-model semantics. The Social Security earnings test that consumes the per-person wage totals stays in simulate.ts and keeps its own registration. Not a verbatim lift, and the difference is where a reader should look. What came across unchanged, operand for operand, is the selection filter (the type test, the two person lookups with neither made defensive — exactly one non-null assertion moved, personById.get(stream.personId)!, while the ! on stateOf lives in the caller’s definition of it and stayed in simulate.ts — stopAge = endAge ?? person.retirementAge, and the combined !alive || (stopAge !== null && ageAttained >= stopAge) gate) and the amount expression (Math.pow(1 + (realGrowthPct ?? 0) / 100, year - startYear) then annualGross * raiseFactor * inflFactor, in that bracketing). Nothing inside was tidied: the realGrowthPct ?? 0 fallback is unreachable through parsePlan, since the schema defaults it to 0 over a non-nullable number, and is kept anyway because this is a behaviour-preserving refactor. Two kinds of evidence for that, kept apart rather than both called measured: MEASURED by an instrumented engine copy, 0 of 77 differential-corpus members reach it; BY CONSTRUCTION, the 400 generated fuzz plans cannot, since the generator always assigns a numeric realGrowthPct and every candidate is then parsed. So the equivalence dump cannot protect that branch; the helper’s own unit test does, by handing it an unparsed stream with the field absent. What changed shape is the output. The inlined phase accumulated into incomes.wages and ordinaryIncome, folded a per-person wagesByPerson map and called recordWages in place, inside its own loop; the helper returns one row per paying stream in plan.incomes order, never sums across them, and leaves all three folds and the recorder call to the caller. Only the ordinaryIncome fold can move a number under re-association — incomes.wages has a single writer and wagesByPerson is rebuilt empty each year, so both are zero-based (injecting a per-person pre-sum moved 0 of 308 corpus entries). Two further shape changes worth naming because neither moves a number. The ledger payload is now built eagerly: the inlined phase built the literal INSIDE yearSites?.recordWages({ … }), and optional chaining does not evaluate a call argument when the receiver is nullish, so wherever yearSites is null it was never constructed. That is every projection which does not ask for the cash-flow ledger, plus every simulatePlan re-entry inside Monte Carlo, the optimizer and the spending solver — but NOT every product projection: the app’s Results page passes captureAnnualCashFlow: true and built these payloads before this change too. Benchmarked at 40M rows per sample with the caller folds present in both arms, the eager arm was the slower of the two on both min and median in 6 of 6 repeat runs on one machine, by 1.3-3.4 ns per paying row; the arms’ ranges overlapped in 5 of those 6, and reviewers re-running the same harness elsewhere saw overlap in every session, so treat a couple of ns per row as an upper bound rather than a constant. The phase yields 0.2754 rows per projected year across the corpus. And the throw on an unknown personId now happens before the caller folds any earlier stream, where the inlined loop folded streams 1..k-1 first; that path is unreachable through parsePlan, which validates the reference. Two contract points a reader should not have to infer: rows are keyed by POSITION and never by stream id, because parsePlan accepts duplicate income-stream ids; and personById is passed as the caller’s map while stateOf is passed as the caller’s function, because the first is last-wins and the second is first-wins and parsePlan accepts duplicate person ids unless a retirement action references them — unifying them is observable, and moves 4 of 308 corpus entries in the member that carries duplicates' }),
