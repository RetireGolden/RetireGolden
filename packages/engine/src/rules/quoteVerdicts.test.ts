...
describe('fetch identity ladder gate', () => {
  it('retries only refused statuses on allowlisted hosts', () => {
    ...
    expect(fallbackEligible('www.michigan.gov', 403)).toBe(true)
    expect(fallbackEligible('www.michigan.gov', 401)).toBe(true)
    expect(fallbackEligible('www.michigan.gov', 406)).toBe(true)
  })
  it('never retries a served or errored response', () => {
    ...
    expect(fallbackEligible('www.michigan.gov', 200)).toBe(false)
    expect(fallbackEligible('www.michigan.gov', 404)).toBe(false)
    expect(fallbackEligible('www.michigan.gov', 500)).toBe(false)
  })
  it('never retries hosts outside the allowlist, jct.gov included', () => {
    ...
    expect(fallbackEligible('michigan.gov', 403)).toBe(false)
    expect(fallbackEligible('legislature.mi.gov', 403)).toBe(false)
  })
})
