...
function structural(value: unknown, seen = new WeakSet<object>()): unknown {
  ...
  if (seen.has(value) || (Array.isArray(value) ? Object.getPrototypeOf(value) !== Array.prototype || Reflect.ownKeys(descriptors).length !== value.length + 1 || Reflect.ownKeys(descriptors).some((key) => key !== 'length' && (typeof key !== 'string' || !descriptors[key]!.enumerable || !Object.hasOwn(descriptors[key]!, 'value') || !Number.isInteger(Number(key)) || Number(key) < 0 || String(Number(key)) !== key || Number(key) >= value.length)) : (Object.getPrototypeOf(value) !== Object.prototype && Object.getPrototypeOf(value) !== null) || Reflect.ownKeys(descriptors).some((key) => typeof key !== 'string' || !descriptors[key]!.enumerable || !Object.hasOwn(descriptors[key]!, 'value')))) throw new TypeError('Employer penalty evidence must be non-aliased acyclic plain data')
  seen.add(value)
