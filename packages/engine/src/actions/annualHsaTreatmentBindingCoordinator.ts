...
function assertRejoin(
  ...
  candidate.applications.forEach((application, index) => {
    ...
    if (entry.qualifiedMedicalAmount + entry.nonqualifiedAmount !== application.stagedAmount || coverage.qualifiedMedicalAmount + coverage.nonqualifiedAmount !== application.stagedAmount || coverage.taxCharacter.reduce((sum, segment) => sum + segment.amount, 0) !== application.stagedAmount || penaltyCoverage.penaltyRelevantCharacterAmount !== application.stagedAmount || penaltyCoverage.coveredPenaltyExposureAmount !== application.stagedAmount || penaltyCoverage.coverageDifferenceAmount !== 0 || penaltyCoverage.evaluations.reduce((sum, item) => sum + item.characterAmount, 0) !== application.stagedAmount) throw new RangeError(`HSA treatment cent coverage mismatch for allocation "${application.allocationId}"`)
  })
