...
describe("canonical scenario patch documents", () => {
  it("round-trips a manually edited plan byte-for-byte after normalization and reverts it", () => {
    ...
    expect(canonicalScenarioJson(applied.plan)).toBe(
      canonicalScenarioJson(edited),
    );
    expect(base.assumptions.inflationPct).toBe(2.5);
    const reverted = revertScenarioPatch(applied.plan, patch);
    expect(reverted.ok).toBe(true);
    if (reverted.ok)
      expect(canonicalScenarioJson(reverted.plan)).toBe(
        canonicalScenarioJson(base),
      );
  });
  ...
  it("holds apply/revert and empty-diff invariants across deterministic generated edits", () => {
    for (let index = 0; index < 40; index++) {
      ...
      expect(canonicalScenarioJson(applied)).toBe(
        canonicalScenarioJson(edited),
      );
  ...
  it("canonicalizes JSON and operation ordering without locale-sensitive comparison", () => {
    expect(canonicalScenarioJson({ z: 1, nested: { b: 2, a: 1 }, a: 0 })).toBe(
    ...
    if (parsed.ok) {
      expect(
        parsed.patch.operations.map((operation) => operation.path),
      ).toEqual(["/assumptions/inflationPct", "/expenses/baseAnnual"]);
    }
  });
  ...
  it("allows unrelated baseline drift but rejects a stale target atomically", () => {
    ...
    if (!rejected.ok)
      expect(rejected.conflicts.map((conflict) => conflict.path)).toEqual([
        "/assumptions/inflationPct",
      ]);
  ...
  it("is idempotent in both directions", () => {
    ...
    if (!second.ok) return;
    expect(canonicalScenarioJson(second.plan)).toBe(
      canonicalScenarioJson(first),
    );
    ...
    if (again.ok)
      expect(canonicalScenarioJson(again.plan)).toBe(
        canonicalScenarioJson(base),
      );
  });
  ...
  it("holds generated conflict and composition invariants", () => {
    for (let index = 0; index < 25; index++) {
      ...
      if (composed.ok)
        expect(canonicalScenarioJson(apply(base, composed.patch))).toBe(
          canonicalScenarioJson(end),
        );
  ...
  it("migrates legacy patches equivalently across generated values", () => {
    for (let index = 0; index < 25; index++) {
      ...
      if (oldApplied.ok && migrated.ok) {
        expect(canonicalScenarioJson(apply(base, migrated.patch))).toBe(
          canonicalScenarioJson(oldApplied.plan),
        );
      }
    }
  });
  ...
  it("rejects protected-field differences and binds plan identity", () => {
    ...
    if (!created.ok)
      expect(created.issues).toContain(
        'protected field "updatedAtIso" differs',
      );
    const migrated = migrateLegacyScenarioPatch(
      base,
      { name: "Renamed through legacy patch" },
      metadata,
    );
    ...
    if (!applied.ok)
      expect(applied.conflicts.map((conflict) => conflict.kind)).toContain(
        "plan-id",
      );
  });
  ...
  it("persists through the plan schema and excludes scenario history from the baseline fingerprint", () => {
    ...
    const applied = applyScenarioPatch(
      reparsed.plan,
      reparsed.plan.scenarios[0]!.patch,
    );
    expect(applied.ok).toBe(true);
...
describe("scenario patch validation and hostile paths", () => {
  ...
  it.each([
    ...
    "rejects unsafe or array-positional path %s during document parsing",
    (path) => {
      expect(
        parseScenarioPatch({
          ...envelope,
          operations: [
            { op: "set", path, before: { present: false }, value: 1 },
          ],
        }).ok,
      ).toBe(false);
    },
