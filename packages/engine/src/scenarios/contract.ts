...
export type ParseScenarioPatchResult =
...
export function parseScenarioPatch(input: unknown): ParseScenarioPatchResult {
  ...
  const operations = [...parsed.data.operations].sort((left, right) =>
  ...
  for (let index = 1; index < operations.length; index++) {
    ...
    if (pathsOverlap(previous.path, current.path)) {
      issues.push(
        `operations overlap at "${previous.path}" and "${current.path}"`,
      );
    }
  }
  return issues.length > 0
    ? { ok: false, issues }
    : { ok: true, patch: { ...parsed.data, operations } };
}

export function isScenarioPatchDocument(
  input: unknown,
): input is ScenarioPatchV1 {
 ...
 */
export function isScenarioPatchEnvelope(
  input: unknown,
