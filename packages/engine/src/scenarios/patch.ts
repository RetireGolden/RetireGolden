...
export function canonicalScenarioJson(value: unknown): string {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value))
      throw new Error("scenario JSON cannot contain a non-finite number");
    return JSON.stringify(value);
  }
...
export function scenarioPlanSnapshotHash(plan: Plan): string {
  ...
  if (!parsed.ok)
    throw new Error(
      `cannot fingerprint invalid plan: ${parsed.issues.join("; ")}`,
    );
...
function valueStateAt(
  ...
  return current === undefined
    ? { present: false }
    : { present: true, value: cloneJson(current) as JsonValue };
}

function assertNoArrayTraversal(
  root: Record<string, unknown>,
  segments: readonly string[],
): string | null {
  let current: unknown = root;
  ...
  if (edited === undefined) {
    ...
    operations.push({
      ...
      before:
        base === undefined
          ? { present: false }
          : { present: true, value: cloneJson(base) as JsonValue },
      value: cloneJson(edited) as JsonValue,
    });
  }
}

export type CreateScenarioPatchResult =
  { ok: true; patch: ScenarioPatchV1 } | { ok: false; issues: string[] };
...
export function createScenarioPatch(
  ...
  const protectedChanges = protectedFields.filter(
    (field) => !jsonEqual(base.plan[field], edited.plan[field]),
  );
  if (protectedChanges.length > 0) {
    return {
      ok: false,
      issues: protectedChanges.map(
        (field) => `protected field "${field}" differs`,
      ),
    };
  }
  ...
  for (const root of editableRoots)
    diffNode(baseRecord[root], editedRecord[root], [root], operations);
...
export function applyLegacyScenarioPatch(
  plan: Plan,
  patch: LegacyScenarioPatch,
): ParsePlanResult {
...
export function migrateLegacyScenarioPatch(
  ...
  if (isScenarioPatchEnvelope(legacyPatch))
    return parseScenarioPatch(legacyPatch);
  const applied = applyLegacyScenarioPatch(basePlan, legacyPatch);
  return applied.ok
    ? createScenarioPatch(basePlan, applied.plan, metadata)
    : { ok: false, issues: applied.issues };
}
...
function operationTarget(operation: ScenarioOperation): ScenarioValueState {
  return operation.op === "set"
    ? { present: true, value: operation.value }
    : { present: false };
}

function stateEqual(
  left: ScenarioValueState,
  right: ScenarioValueState,
): boolean {
  return (
    left.present === right.present &&
    (!left.present || (right.present && jsonEqual(left.value, right.value)))
  );
}
...
function conflictReport(
  ...
  for (const operation of patch.operations) {
    ...
    const expected =
      direction === "apply" ? operation.before : operationTarget(operation);
    const idempotent =
      direction === "apply" ? operationTarget(operation) : operation.before;
  ...
  return {
    baseSnapshotMatches:
      scenarioPlanSnapshotHash(plan) === patch.base.snapshotHash,
...
function mutateOperations(
  ...
  if (!parsedPatch.ok)
    return { ok: false, issues: parsedPatch.issues, conflicts: [] };
  ...
  const operations =
    direction === "apply"
      ? parsedPatch.patch.operations
      : [...parsedPatch.patch.operations].reverse();
  for (const operation of operations) {
    const segments = decodeScenarioPointer(operation.path)!;
    const target =
      direction === "apply" ? operationTarget(operation) : operation.before;
    const issue = target.present
      ? setAt(draft, segments, target.value)
      : removeAt(draft, segments);
    if (issue) {
      return {
        ok: false,
        issues: [issue],
        conflicts: [
          { kind: "invalid-path", path: operation.path, message: issue },
        ],
      };
    }
  }
...
export function applyScenarioPatchDocument(
  plan: Plan,
  patch: ScenarioPatchV1,
): ApplyScenarioDocumentResult {
...
export function revertScenarioPatch(
  plan: Plan,
  patch: ScenarioPatchV1,
): ApplyScenarioDocumentResult {
