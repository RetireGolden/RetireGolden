...
| `rules/` | `TAX_RULE_REGISTRY` — every statutory rule the engine implements, as data: the authority it rests on, the reading taken, the contrary reading where one exists, and the date it was last verified against primary sources. Its own subpath, not the root export, because the records carry quoted statutory text. The directory also holds the calculation catalog (`CALCULATION_REGISTRY`, its evidence-fixture helper, and the generated output-family census), which is test and tooling data excluded from `dist`, not part of the subpath |
