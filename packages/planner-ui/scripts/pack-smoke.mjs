...
const exportsSmoke = [
  ...
  '',
  '// The loop above only proves every key PRESENT in the packed manifest',
  '// resolves -- it has nothing to say if a legitimate key went missing,',
  "// since a sweep over the manifest's own contents cannot see a key that",
  '// is not there. This is the exact 36-key deep-subpath set as of 0.10.0:',
  '// the 13 stability-promised subpaths under README "Published API surface"',
  '// plus the 23 unpromised deep subpaths in its "Unpromised deep subpaths"',
  '// table. Keep this list, the package.json exports map, and the README',
  '// table in sync.',
  'const expectedDeepSubpaths = [',
  "  './plan-format',",
  "  './complete-export',",
  "  './plan-tax-calculator',",
  "  './projection',",
  ...
  "  './index.css',",
  "  './data/localStore',",
  "  './data/planStoreContract',",
  "  './data/v2Backup',",
  "  './householdMap/householdGraph',",
  "  './householdMap/mapViewModel',",
  "  './import/brokerCsv',",
  "  './import/genericCsv',",
  "  './import/projectionLab',",
  "  './import/reviewChecklist',",
  "  './import/tenForty',",
  "  './learn/learningRegistry',",
  "  './optimize/runOptimize',",
  "  './planner/examples/buildContext',",
  "  './planner/examples/buildExampleCouple',",
  "  './planner/examples/buildUnderSavedSingle',",
  "  './planner/examples/registry',",
  "  './planner/format',",
  "  './planner/planContextCore',",
  "  './planner/refreshProtectionContext',",
  "  './planner/useProjection',",
  "  './report/reportHtml',",
  "  './routes/LearnRoutes',",
  "  './routes/groups',",
  ']',
  'const actualDeepSubpaths = Object.keys(exported).filter(',
  "  (key) => key !== '.' && key !== './package.json' && exported[key] !== null,",
  ')',
  'const missingKeys = expectedDeepSubpaths.filter((key) => !actualDeepSubpaths.includes(key))',
  'const extraKeys = actualDeepSubpaths.filter((key) => !expectedDeepSubpaths.includes(key))',
  'if (missingKeys.length > 0 || extraKeys.length > 0) {',
  '  fail(',
  "    'exports map deep-subpath set changed -- missing [' + missingKeys.join(', ') + '], extra [' +",
  "      extraKeys.join(', ') + '] -- update pack-smoke.mjs and the README table together',",
  '  )',
  '}',
  '',
