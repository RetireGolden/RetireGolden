 ...
 *
 * **The design record lives in `DOCS/features/document-text-extraction.md`**:
 * why `pdfjs-dist` is an optional peer a host may also hand in, why the
 * dynamic import below resolves a bare specifier and what that means per
 * runtime, how the worker-free path is constructed and why an unset
 * `globalThis.pdfjsWorker` is not refused up front, what the caps do and do
 * not bound, how the local-only guarantee is enforced through pdfjs's own
 * options, and why a page number is deliberately not a `SourceLocator` kind.
 * The measured results, and the reason vocabulary as a user meets it, are in
 * `DOCS/features/document-parsing-spike.md`. What stays below is what a reader
 * editing this file has to hold in mind.
 *
 * **Stability promise:** this module is published as the
 * `@retiregolden/planner-ui/document-text` subpath and, unlike the unpromised
 * deep subpaths, is a supported API: the exported names, their signatures, the
 ...
 *
 * **Never write the pdfjs package specifier inside an `import(` call, in code
 * or in prose.** It is spelled outside the call throughout this file, comments
 * included: the source-scan test forbids a literal `import(` of the package
 * name anywhere in this source, and a guard that has to reason about which
 * occurrences are comments is a guard that can be argued with. See
 * {@link PDFJS_ENTRY} for how the entry point is assembled instead.
 *
 * **Three host conditions, three reasons, and none of them blames the user's
 * document.** `pdfjs_unavailable` (the package is not installed),
 * `pdfjs_worker_unavailable` (a pdfjs is present but has no worker to run in;
 * see {@link isWorkerSetupFailure}), and `pdfjs_incompatible` (the build
 * present does not expose the API this module drives). Their remedies differ,
 * so reporting the wrong one is a false statement about the host's setup.
 *
