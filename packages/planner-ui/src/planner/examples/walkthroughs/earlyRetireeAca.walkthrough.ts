...
export const EARLY_RETIREE_ACA_WALKTHROUGH: Walkthrough = {
  ...
  tables: [{
    ...
    rows: [
      { key: 'age', label: "Casey's age attained", hand: 62, derivation: '2026 − 1964', contract: 'PersonYearState.ageAttained', select: (year, plan) => year.people.find((p) => p.personId === plan.household.people[0]!.id)?.ageAttained },
      ...
      { key: 'aca-fpl-pct', label: 'MAGI as a percentage of the poverty line', hand: FPL_PCT, derivation: '28,500 ÷ 15,650 × 100 = 182.11', contract: 'worksheet aca-expected-contribution; YearAcaResult.fplPct', select: (year) => year.aca?.fplPct },
