...
describe('Household unnamed people (#523)', () => {
  it('the fallback reads as a placeholder, not "Person"', () => {
    expect(fallbackPersonName(0)).toBe('Unnamed person')
  ...
  it('a cleared name is stored as the placeholder, and the field hint says so', () => {
    ...
    typeInto(primary!, '')
    expect(applied.at(-1)!.household.people[0]!.name).toBe('Unnamed person')
    ...
    expect(el.textContent).toContain('Blank = shown as Unnamed person.')
  })

  it('the card header always shows the stored name, placeholder or not', () => {
    const plan = createSamplePlan()
    ...
    expect(titles).toContain('PartnerUnnamed partner')
