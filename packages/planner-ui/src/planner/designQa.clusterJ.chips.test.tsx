 ...
 *
 * The source half — that the badge markup has exactly one home, so no call
 * site can go back to a bare `<span className="type-chip">` — needs the node
 * environment and lives in designQa.clusterJ.test.ts.
 */
...
/** Everything that reads before / after this badge inside its own parent element. */
function textBeforeChip(chip: Element): string {
  let text = ''
  for (let node = chip.previousSibling; node !== null; node = node.previousSibling) text = (node.textContent ?? '') + text
  return text
}
...
/**
 * The rule, applied to whatever the section rendered, on both sides: a badge
 * either ends (or starts) its container, or the text next to it begins with
 * whitespace. Returns the offending glued strings so a failure names them the
 * way the walk did.
 */
function gluedBadges(host: HTMLElement): string[] {
  ...
  for (const chip of host.querySelectorAll('.type-chip')) {
    ...
    const before = textBeforeChip(chip)
    if (before !== '' && !/\s$/.test(before)) glued.push(`${before.slice(-40)}${label}`)
    const after = textAfterChip(chip)
    if (after !== '' && !/^\s/.test(after)) glued.push(`${label}${after.slice(0, 40)}`)
  }
  return glued
}
...
describe('cluster J: the kind badge is separate in the text layer too (#570)', () => {
  ...
  it('Social Security rows keep the badge off the person name', async () => {
    const host = await mount(validPlan(), <SocialSecuritySection />)
    expect(gluedBadges(host)).toEqual([])
  })
})
