...
function rule(selector: string, source = css): string {
  const start = source.search(new RegExp(`(?:^|[\\s}])${escapeRegExp(selector)}\\s*\\{`, 'm'))
  return ruleBodyAt(source, start, selector)
}
...
describe('Design-QA chrome pins', () => {
  ...
  it('survivor table headers get a column floor and never break mid-token (#431)', () => {
    expect(rule('.compare-table.survivor-table')).toMatch(/min-width:\s*66rem/)
