...
- **Spending-solver worker TDZ (#672):** the planner Web Worker no longer isolates
  ...
  `Cannot access 'oe' before initialization`, shown as "Solver error" on
  How much can I spend?. Kernels and publications stay split. The bundle-budget
  gate now fails if any other chunk statically imports the worker entry.
